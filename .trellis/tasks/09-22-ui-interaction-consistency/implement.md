...
进度记录（随评审修订更新）：W1–W10 全部十个叶子已合入并归档。
W1 以 PR #56/#57 收尾（main@b8eafb3）；W6
（entity-management-consistency）按对象/管理/下载三波合入，末波为
PR #83（管理）与 #84（下载）；W10（motion-integration-acceptance）
以 PR #86 合入；各叶子的评审修复（#73–#78、#80–#82）均已落地。
剩余收口：`09-24-interaction-followups` 承载验收期确认的十项窄修复。
父任务持续只承载簿记与最终集成，不含产品代码。
