...
- [x] `docs(09-16): novel repair 根因证据与设计固化`——提交本任务 prd/design/implement + 父任务与全部子任务规划文档（随本 PR 入库，沿用 09-02 先例）
...
- [x] `feat(novel): 收藏 add/delete 与系列 prev/next 兜底`——BookmarkRepository/Store 扩展 + 详情页入口
- [x] `feat(novel): 评论支持 novel 维度`——CommentStore (workKind,workId) 泛化 + repository 端点 + 详情页评论入口
- [x] `feat(novel): 小说排行与热词`——/v1/novel/ranking feed + ranking 页入口 + trending-tags/novel
