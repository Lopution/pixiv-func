...
- [ ] 用一张 Pixiv 作品的截图搜索，能命中该作品，点击进入站内详情页。
- [ ] 用一张非 Pixiv 来源的图搜索，结果可点击并由系统浏览器打开。
- [ ] 触发限流时显示「可重试 + 等待时间」，而不是泛化的失败文案。
- [ ] 服务端返回 HTML / 挑战页时显示明确失败，不显示空结果列表。
- [ ] 「无匹配结果」与「请求失败」在界面上是两种不同的呈现。
- [ ] 取消 / 失败 / 成功三条路径均已清理临时文件（扩展现有的 exactly-once 清理测试）。
- [ ] 全流程不要求用户填写任何 key 或注册任何账号。
- [ ] `flutter analyze` 与 `flutter test` 通过。
- [ ] 真机验证清单交付（结果跳转、限流提示、隐私提示可见）。
