...
- [x] `feat(series): 系列 feed 与系列页`——_IllustSeriesFeedController/_UserSeriesFeedController + illustSeriesContextProvider + IllustSeriesPage(头+IllustFeedGrid+触底/刷新)+ UserSeriesFeed 卡片网格 widget + `series/:seriesId` 路由与 openIllustSeries + feed/widget 测试
- [x] `feat(series): 详情页与用户页入口`——IllustSeriesSection sliver 接入详情页 + ProfileWorkSection{illust,manga,novel,series} 替换 _workType + tabs delegate 第 4 chip + UserSeriesFeed 挂入 _ProfileTabBody + l10n 四语言 + widget 测试
