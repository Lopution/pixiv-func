...
- 进入代表作品作为次要操作保留（现为长按）。
- 反复进出搜索页不重复请求 trending-tags。当前 provider 是 `autoDispose`，每次进入都会
  重新请求一次。
- 不为每个标签单独发搜索请求，不引入客户端随机算法，**也不为此新增日期判定或持久化
  缓存层** —— 跨日刷新由 App 重启自然发生，多显示一天旧标签没有产品后果。
...
- 反向回滑期间**只有指示器在动，页面内容不上滚**；指示器完全消失后，剩余手势才开始滚动
  列表内容。
...
- [x] 详情页点击作者头像、用户名或 account 任一处，都进入该作者的用户页。
- [x] 登录页设置读取中显示加载态；读取失败显示错误与重试，点重试真实重新读取。
- [x] UI 语言切到日语与英语，详情页 9 处文案全部随之改变，无残留中文。
...
- [x] 点击卡片进入该标签的搜索结果。
- [x] 次要操作可进入代表作品。
- [x] 反复进出搜索页只请求一次 trending-tags。

### 阶段 3（D7 / U8）

- [x] 展开态头像直径落在 96–112dp。
- [x] 从展开滚到折叠，头像尺寸与位置连续变化，没有突然替换的瞬间。
- [x] 头像在过渡全程可见，不随背景图一起淡出。
- [x] 折叠后用户名出现在 toolbar 位置。
- [x] 无头像用户不再出现巨大占位淡出的观感。

### 阶段 4（U1）

- [x] 下拉引出图标后反向回滑，图标跟随手指连续回落。
      实测：从 armed 反向回滑，图标随手指连续上移，无两态跳变；到满程 2/3 处停住
      （框架 `_kDragSizeFactorLimit` 的 armed 地板），剩余收起动作在松手时完成。
      用户报告的「图标不跟随、与列表一起上移」已消除。
- [ ] **未达成**：回滑期间页面内容不上滚；图标完全消失后，剩余手势才开始滚动列表。
      框架标准 `RefreshIndicator` + `ClampingScrollPhysics` 做不到：下拉期间
      `pixels` 恒为 0，反向手势的位移会**直接**进入 `applyUserOffset`，
      于是「图标回落」与「列表上滚」同时发生（实测 rev 20 → `pixels=20`）。
      要让反向手势先被指示器「吃掉」，只能重新拦截 drag —— 即 `design.md` 5.3
      硬约束明令禁止的第二套状态机（这正是本次事故的根因）。
      按 5.3「若某个视觉诉求只能靠自建状态机实现，放弃该视觉诉求，不放弃正确性」，
      本项**建议放弃**，待用户真机体验后裁决；若用户坚持，需要单独立项重新评估代价。
- [x] 手指离开屏幕后的惯性滚动与 overscroll 不再唤出图标。
- [x] 快速多次下拉、下拉后甩动、连续第二次刷新，图标都不残留在屏幕上。
- [x] 达到阈值释放仍然真实触发一次刷新，且只触发一次。
- [x] 未达阈值释放不触发刷新。

### 全阶段

- [x] `flutter analyze` 与 `flutter test` 通过（四阶段全部；538 tests，末轮无 loopback 噪声）。
- [ ] 需要人眼判定的项（阶段 3 过渡观感、阶段 4 手势手感）在四阶段全部完成后，
      由用户在自己的手机上验证 —— 2026-09-01 用户决定不做逐阶段模拟器截图留证。
- [x] 不修改网络、下载、账号相关代码；本 child 不触碰其它 child 的范围。
