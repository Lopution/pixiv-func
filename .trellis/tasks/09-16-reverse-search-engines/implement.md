...
- [ ] `feat(reverse-image): 引擎模型与 IQDB provider`——`ReverseImageEngine`/`ReverseImageEngineSpec`/`unsupportedInput`/`ReverseImageSearchWebUpload` outcome + 共享 headless 分类提取 + `IqdbWebViewProvider`（8MiB/jpeg-png-gif 门控）+ 单测
