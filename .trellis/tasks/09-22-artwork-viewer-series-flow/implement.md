...
- [x] 新建 `lib/app/haptics/app_haptics.dart`：`abstract final class
...
- [x] `lib/core/settings/app_settings.dart` 增 `enableHaptics = true`
...
- [x] `browse_settings_page.dart` 增一行 `SettingsControl`（接
...
- [x] 首个消费者触点接入（与 snackbar/状态变更同点同分支）：
...
- [x] `illust_detail_page.dart`：AppBar `downloadAll` 解除
...
- [x] `page_image.dart`：`DetailPageImage` 增 `selected`/`onToggleSelect`；
...
- [x] `info_block.dart`：`TagChip.onLongPress` 从 `_blockMode` 翻转改为
...
- [x] `ugoira_viewer.dart`：导出钮脱离 `downloadMode && _asset != null`
...
- [x] 旧测试同 commit 改写：`illust_detail_page_test.dart` 下载模式组
...
- [x] `image_viewer_page.dart`：`StatefulWidget`→`ConsumerStatefulWidget`，
...
- [x] chrome 显隐：`_chromeVisible` 提升为**会话级 holder**（viewer 会话
...
- [x] 双击缩放循环 + 单击消歧：同一 `GestureDetector` 注册
...
- [x] 底栏工具栏：左 `n/total`（InkWell → `showAppBottomSheet` 页码
...
- [x] PopScope：`canPop = !_activeZoomed`（chrome 隐不阻塞退出）；
...
- [x] `illust_detail_page.dart` 窄屏分支：AppBar 下固定 compact header
...
- [x] `series_models.dart` `IllustSeriesEntity` 增 `firstContentId`
...
- [x] 新建 `lib/core/series/series_recent_open_store.dart`：
...
- [x] `illust_series_page.dart` `_SeriesHeader` 增动作行：「开始阅读」
...
- [x] **单击/双击序列**：单击→等超时→chrome 切；双击→缩放、chrome 不动；
      双击后再单击→chrome 切（risks R1）。
- [x] **PopScope × route-replace**：放大→滑动翻页（`replace` 重建 route）
      →返回应退路由（State 已重置）；同页内放大→返回→复位不退；
      **chrome 隐→翻页→仍隐；chrome 隐→返回→直接退**（risks R2）。
- [x] **l10n arb 并行冲突**：与并行叶子的 arb/生成物冲突时，arb 并集
      保留双方 key → `flutter gen-l10n` → `gen_l10n_lookup.py`，生成物
      不手解（design.md §六）。
- [x] **下载模式 chrome**：窄屏纵列/双栏 pager/横屏三形态呈现断言；
      全屏图片作品退出路径=取消钮（risks R4）。
- [x] **降级态**：entity null/Restricted/Error 快照下 compact header、
      查看器动作、系列动作行的渲染规则逐一断言。
- [x] **触觉双路径**：开关开→观察器收级别序列；开关关→零调用且
      snackbar/角标等视觉反馈不变。
- [x] **运行时矩阵**（PR 标注真实结果或「未验证」）：320/390/600/840/
...
- [x] `flutter analyze --no-pub` 与全量 `flutter test` 通过（噪声按
      spec 判定）；`git diff --check` 干净；`task.py validate` 通过；
      `grep -rn "HapticFeedback" lib/` 仅命中 `app_haptics.dart`。
- [x] PR：`gh pr create --fill`；CI 绿后 `gh pr merge --merge`。
- [x] 收尾记账：`add_session.py` + `task.py archive`（随本 PR 的最后
      提交）。
