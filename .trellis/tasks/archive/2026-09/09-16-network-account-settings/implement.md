...
- [x] `feat(backup): 导出导入域与 JSON schema`——`core/backup/`：`pixivfunc.backup.v1` envelope（settings/mutes/history）+ 导出服务（prefs→SAF 文件）+ 导入服务（openFile→校验→merge/overwrite 策略）+ `BackupImportException` + 单测
- [x] `feat(backup): 导出导入 UI`——设置入口 + 合并/覆盖对话框 + 结果反馈 + l10n + widget 测试
