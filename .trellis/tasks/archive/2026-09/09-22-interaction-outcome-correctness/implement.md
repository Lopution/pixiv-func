...
- [x] **R1**：`lib/features/novel/novel_page.dart` L352 与 L133 的 `maybePop()` →
...
- [x] **R2**：同文件 `NovelInfoSheet` tags Wrap（L567-573）`TagChip` 接 `onTap`：
...
- [x] **R3**：新增 `lib/core/localnovel/read_offset_anchor.dart` 纯函数
...
- [x] **R5**：`lib/features/profile/profile_edit_page.dart` `_attemptPop`（L137-167）
...
- [x] **R4**：`lib/features/search/search_page.dart` L153-155 `itemCount: tags.length`，
      删除裁剪逻辑与注释。
      测试：`test/search_page_test.dart` 4/5 个 trending tag 全渲染断言。
      提交：`fix(search): 热门标签不再为凑整行丢内容`
- [x] **R6**：`lib/features/settings/pages/browse_settings_page.dart` "测试"按钮改名
      "应用并测试"（新 l10n key，四语），行为不变。
      测试：`test/settings_test.dart` 断言点击后 repository 值按新文案语义更新。
      提交：`fix(settings): 镜像源测试改名应用并测试`
- [x] **R7**：`lib/features/settings/pages/translation_credentials_page.dart` `_clear`
...
- [x] **R8**：`lib/core/reverse_image/reverse_image_controller.dart` 新增 `stopSearch()`
...
- [x] **R9**：两端 `login_webview*_page.dart`：可恢复错误卡加"重新加载"（`reload()`）；
