# 真实高频 UX 正确性修复

## Goal

修复用户在真实设备上每天都会碰到的界面与交互错误。本 child 不做 UX 重构，也不碰网络、
下载、账号逻辑：它是一组边界清楚的垂直修复，加上最后一项共享滚动基建的收敛。

覆盖审计编号：U1, U2, U8, U9, D7, C11, C20（限本 child 触及的页面）。

## Requirements

### R1. 作品详情页作者区域可点击（U9）

现状：详情页作者区域（头像 + 用户名 + account）是一个纯 `Row`，没有任何点击响应。
`showUserPage` 已在同文件被 import 并使用，但只服务于简介正文里的 pixiv 用户链接。

- 头像、用户名、account 构成的整个区域可点击，进入该作者的用户页。
- 沿用现有的 `showUserPage(context, userId)`，不改路由架构。
- 保留区域现有的 `Key('illust-author-avatar')`，不破坏已有测试定位。

### R2. 登录页设置读取失败不再白屏（C11）

现状：设置读取的 loading 与 error 分支都返回空 `ReplicaScaffold`，用户看到一片空白，
既不知道在加载还是失败，也没有任何恢复手段。

- loading：显示正常的加载指示。
- error：显示错误信息与「重试」，重试真实重新读取设置。
- 这是应该做的真实错误处理，不是防御式编程；不要用「回退到默认设置继续渲染」掩盖失败。

### R3. 详情页硬编码中文进 i18n（C20）

现状：`illust_detail_page.dart` 共 9 处直接写死中文，UI 语言切到日语/英语/俄语时仍显示中文。
涉及：页面标题回退、投稿日期（未知 / 有值两种）、尺寸、打开链接失败提示、作品受限、
作品不存在、加载失败、重试按钮。

- 全部进现有 `ReplicaStrings`，四种语言（zh / en / ja / ru）都要补齐，不留 TODO。
- 不更换 i18n 框架。
- 本 child 只处理详情页；其它页面的硬编码中文不在本 child 范围内。

### R4. 搜索热门标签显示代表图（U2）

现状：`/v1/trending-tags/illust` 已返回 representative illust，Func 已解析并写入
`IllustStore`，但卡片只显示一行标签文字，代表作品仅在长按时使用。

- 卡片展示 API 自带的 representative 图片。
- 点击卡片 = 搜索该标签（保持现有主操作语义不变）。
- 进入代表作品作为次要操作保留（现为长按）。
- 反复进出搜索页不重复请求 trending-tags。当前 provider 是 `autoDispose`，每次进入都会
  重新请求一次。
- 不为每个标签单独发搜索请求，不引入客户端随机算法，**也不为此新增日期判定或持久化
  缓存层** —— 跨日刷新由 App 重启自然发生，多显示一天旧标签没有产品后果。

### R5. 个人主页 Header 连续过渡（D7 / U8）

现状：头像和用户名分别在展开态与折叠态重复渲染，滚动临界点会发生替换；展开态
头像还与名字共用一条整体淡出曲线，导致无头像占位显得过大，并在滚动时与 toolbar
名字发生冲突。

- 头像只属于展开态身份区，直径固定为约 104dp（落在 96–112dp 目标区间），随身份区
  一起滚出，不再缩进 pinned toolbar。
- 展开头像与用户名保持固定间距，任意滚动位置都不能重叠；身份区离场后才显示
  toolbar title。
- 背景图独立淡出；头像不作为背景图的透明度子树，也不作为背景水印残留。
- 折叠 toolbar 只保留返回、标题和操作控件，不重复放头像。

### R6. 下拉刷新手势收敛（U1，P0，独立阶段）

现状（真机复现）：
- 下拉引出图标后反向回滑，图标不跟随手指回落，而是与列表一起上移。
- 手指已离开屏幕后，惯性 / overscroll 仍可能重新唤出刷新图标，甚至让图标停在屏幕上不消失。

- 采用 PixEz 同类的 `easy_refresh` 共享封装和 Material header，由组件单独持有刷新
  阈值与生命周期，不再并存第二套阈值。
- 只配置 indicator 的外观，不在业务 wrapper 内重新实现 scroll lifecycle。
- 反向回滑期间**只有指示器在动，页面内容不上滚**；指示器完全消失后，剩余手势才开始滚动
  列表内容。
- 手指离开屏幕后产生的 overscroll 不得重新进入下拉跟踪状态。
- 任何一次下拉结束（刷新或取消）后，指示器必须回到不可见状态，不允许残留。

> 顺序约束：本项必须在本 child 内最后实施，并作为独立阶段独立验收。
> `lib/app/pull_to_refresh.dart` 被全部 feed 页共用，且
> `09-01-settings-productization` 的 C9「过滤后自动续拉填满一屏」要以本项完成后的
> 滚动基建为准 —— 地基未换完不设计续拉行为。

## Acceptance Criteria

### 阶段 1（U9 + C11 + C20）

- [x] 详情页点击作者头像、用户名或 account 任一处，都进入该作者的用户页。
- [x] 登录页设置读取中显示加载态；读取失败显示错误与重试，点重试真实重新读取。
- [x] UI 语言切到日语与英语，详情页 9 处文案全部随之改变，无残留中文。

### 阶段 2（U2）

- [x] 搜索页热门标签卡片显示代表图。
- [x] 点击卡片进入该标签的搜索结果。
- [x] 次要操作可进入代表作品。
- [x] 反复进出搜索页只请求一次 trending-tags。

### 阶段 3（D7 / U8）

- [x] 展开态头像直径落在 96–112dp。
- [x] 展开头像与身份区一起滚出，过程中不缩进 toolbar，也没有两套头像突然替换。
- [x] 头像与展开态名字在任意滚动位置不重叠；背景图独立淡出。
- [x] 身份区退出后用户名出现在 pinned toolbar 位置。
- [x] 无头像用户不再出现巨大占位淡出的观感。

### 阶段 4（U1）

- [x] 下拉引出图标后反向回滑，图标跟随手指连续回落。
- [x] 回滑期间页面内容不上滚；图标完全消失后，剩余手势才开始滚动列表。
- [x] 手指离开屏幕后的惯性滚动与 overscroll 不再唤出图标。
- [x] 快速多次下拉、下拉后甩动、连续第二次刷新，图标都不残留在屏幕上。
- [x] 达到阈值释放仍然真实触发一次刷新，且只触发一次。
- [x] 未达阈值释放不触发刷新。

### 全阶段

- [x] `flutter analyze` 与 `flutter test` 通过（四阶段全部；末轮结果以实际命令输出为准）。
- [ ] 需要人眼判定的项（阶段 3 过渡观感、阶段 4 手势手感）在四阶段全部完成后，
      由用户在自己的手机上验证 —— 2026-09-01 用户决定不做逐阶段模拟器截图留证。
- [x] 不修改网络、下载、账号相关代码；本 child 不触碰其它 child 的范围。

## Notes

- 上级需求与跨 child 约束见 `../09-01-func-1-0-hardening/prd.md`。
- 审计断言复核（含各处证据行号）见
  `../09-01-func-1-0-hardening/research/audit-verification.md`。
- 技术设计见 `design.md`，执行顺序与验证命令见 `implement.md`。
