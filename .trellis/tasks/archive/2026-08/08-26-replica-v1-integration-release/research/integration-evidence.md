...
### API 36 physical-device preflight (network validation paused)

2026-08-29 已通过局域网 ADB 连接真实 `RMX5200`（product
`RE6030L1`），serial 为 `192.168.1.64:41723`，`get-state=device`，SDK
`36`。这证明 API 36 物理设备可达，但当时的清洁网络门禁未通过：
`dumpsys connectivity` 发现活动 VPN owner
`com.github.metacubex.clash.meta`、`tun0=172.19.0.1` 和应用代理
`127.112.154.72:45555`；全局 `settings get global http_proxy` 虽为 `null`，
仍不能视为 proxy/VPN off。因此在用户关闭 Clash/VPN 前没有把该设备用于
Pixiv API、OAuth、图片、下载、WebView 或 Widget headless 结果，也没有把它
写成 Mainland no-external-proxy 通过证据。
