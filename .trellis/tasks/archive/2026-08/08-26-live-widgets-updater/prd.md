...
- 08-26-android-platform-parity、profile-follow-social、downloads-ugoira-media、正常网络主链与 P0 `restricted-compat-network` 完成。
...
- R6: Live endpoint证据先于player依赖；Widget使用versioned secret-free snapshot、经实测的headless复用路径、unique work和RemoteViews IPC预算；Updater只信任signed manifest并校验size/hash/package/APK signer，F-Droid编译期无updater graph。
- R7: Live 与 Widget 的 Pixiv 请求统一复用 `NetworkAccessPolicy`；Updater、F-Droid 与非 Pixiv provider 不进入 compatibility。集成验收必须分开记录系统 proxy/VPN off 的前台/后台出口。
...
- [ ] Live dependency gate、Widget snapshot/account revision/last-good与signed updater invalid-signature/fail-closed矩阵通过。
- [ ] Live/Widget route 与大陆 access evidence 可追溯，headless 失败不会复制 secret 或退化为 native fixed IP/proxy。
