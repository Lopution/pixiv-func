...
- R6: Follow mutation保持account/revision绑定和server-confirmed语义，不默认离线重放；Profile edit使用server capabilities、minimal typed patch和confirmed/verification-pending结果，密码不得进入持久draft。
- R7: `profile-edit` 排在 P0 `restricted-compat-network` 之后；Profile/API/Web/image 统一消费 shared `NetworkAccessPolicy`，不自行创建代理、固定 IP 或 host rewrite。
...
- [ ] Follow晚到/429/账号切换与Profile capability×dirty-field组合、verification pending、password no-persist矩阵通过。
- [ ] 在系统 proxy/VPN 关闭的设备上，Profile 资料/API/Web/image 的 route/failure 与大陆 access evidence 可追溯。
