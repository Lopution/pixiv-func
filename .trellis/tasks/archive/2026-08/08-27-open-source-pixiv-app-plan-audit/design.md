...
- 大陆 access 是 shared transport outcome，不是 feature URL rewrite：exact-host registry、failure taxonomy、direct-first、secure resolver 与 original-host TLS 由兼容网络 leaf 统一拥有，API/OAuth/image/download/Widget 逐出口验收。
- **2026-08-29 修订**：ECH 与 WebView loopback 两条路径的实现已删除，不再是本项目方案。参考实现的直连能力来自「关证书校验 + 省 SNI + 固定 IP」三件套，本项目只采纳其中的省 SNI，且必须自行完成证书链验证与 SAN 核对；固定 IP 降为解析失败后的兜底；关闭证书校验、改写 Host 与第三方反代继续拒绝。
...
本任务只保存链接、事实摘要、状态机和测试思想。GPL 源码不复制；MIT 代码未来若复用也需单独 provenance。第三方秘密、关闭证书校验、BODY 日志、整文件 bytes 和 destructive migration 都列为拒绝证据。
