...
- R6: Ranking/New/Search 的 request、entity merge 与 cursor commit 必须绑定同一 generation；refresh 取消旧 append，旧代结果即使晚到也不能污染共享 store。Reverse Image 必须经过 structured/WebView/unavailable capability gate，WebView 体验差异不得静默接受。
...
- [ ] refresh-wins、same/new/deleted/reordered/disjoint IDs、筛选切换与取消后晚到响应均不写旧entity/cursor；Reverse Image capability结论有观测日期。
