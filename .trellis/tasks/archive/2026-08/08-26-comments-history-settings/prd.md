...
- R6: Comments等非幂等写操作不得在失败/重启后隐式自动重放；History/Settings restore、cancel与dispose有明确所有权，DB升级和降级都不得 destructive migration 或静默清空。
...
- [ ] 进程重启期间restore/remove竞态、DB upgrade/downgrade失败、comment late response/account switch均有确定测试且不误删/重放。
