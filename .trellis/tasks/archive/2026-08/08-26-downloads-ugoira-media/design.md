...
- 媒体网络只能从 shared `NetworkAccessPolicy`/destination registry 取得；兼容网络 leaf 接入后，Download/Ugoira 的 image/archive/redirect 全部复用同一 strict route，不复制 fixed IP 或代理设置。
