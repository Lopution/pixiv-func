...
- [ ] D-1 APK keystore 生成与保管方案已确认，真实密码通过外部 secret 注入。
- [ ] D-2 独立 manifest signing key 方案已确认；不与 APK keystore 共用。
- [ ] D-3 已在 API 29 provider 上确认算法与签名编码；保留可运行 fixture。
- [ ] D-4 已决定手工 release 或 GitHub Actions；D-5 已决定 `fdroid` 是否发布。
- [ ] `design.md` 的 canonical manifest bytes、错误 code、host/path 合同完成 review。
...
- [ ] 搜索 git diff、APK、build 输出和 CI 日志，确认无私钥、密码、token。
- [ ] 用户真机完成 API 29、高版本自更新和 F-Droid/store-managed 行为确认。
