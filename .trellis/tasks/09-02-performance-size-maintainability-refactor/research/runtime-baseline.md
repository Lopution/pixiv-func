# 运行时性能基线（协议备用；D-9：不作为 gate）

用户于 2026-09-07 决定不做运行时基线（D-9）。本文件保留测量协议与代码事实供日后需要时使用；R9 现为静态可验证的
设计约束（parent design §14），任何 child 都不以本文件的数字作为验收条件，也不得声称"更省/更快"。模拟器没有可用的 Pixiv 网络与真实账号，agent 不能代测（见 09-01 parent 2026-09-01 决定）。
