...
`research/dart-architecture-audit.md`、`research/modernization-gaps.md`；规则与目标形态见 parent `design.md` §2–§3、§13–§14。
已确认决策 D-6（gen-l10n）、D-9（不做基线）、D-10～D-13（交互/视觉在 child F，本 child 保持行为中性并交付 enabler）。
...
- 导航合并到 `lib/app/navigation/`：`ReplicaPageRoute` 唯一（删 `replicaRoute()`）、`routes.dart` 门面（以 id/参数为形参：
  `openIllust(context, id)`、`openUser(context, id)`、`openSearch(context, query)`…，F 将用 go_router 重实现而调用点不变；本 child 不引入 go_router）、
  `HomeShellMetrics` 静态量改 provider。
...
- 新组件需 ≥3 处现存重复作为证据（`NovelCard`/`UserCard` 先盘点）。交互/视觉变化（M3、go_router、恢复、Predictive Back、
  Hero 手势、M3 组件）全部属于 child F；本 child 只交付 enabler。

### R8. 性能边界（parent R9，design §14；D-9 不做基线，全部为静态可验证约束）

- feed/avatar 变体必须带按布局尺寸计算的 `memCacheWidth`（布局宽 × dpr，上限 1.5× 逻辑像素），detail 按屏宽，viewer 不限；
  widget 测试断言变体产出的 `ImageProvider` 带尺寸。
- 共享组件只 `watch` 所需 provider 切片（`select`）；`IllustCard` 不 `watch` 整个 feed state；code review 检查。
- 首帧前等待项固定为 settings + 账号；新增初始化 `unawaited`/后置；`startup_gate_test` 断言首帧前不等待网络/widget/rhttp。
- `DownloadRecoveryStore` 设记录上限（淘汰已完成项）或改增量写入，code review 决定；history 三条查询在单测中
  `EXPLAIN QUERY PLAN` 断言不出现 `SCAN TABLE`。
- 不建请求缓存层；不做任何数值化性能结论。
...
- [ ] R8 静态约束的测试全部存在并通过：变体 `memCacheWidth` 断言、`startup_gate_test` 首帧等待项断言、history 查询计划断言；
      `DownloadRecoveryStore` 上限或增量写入已落地。
