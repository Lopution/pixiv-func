...
- [ ] C7b `history_page` → `HistoryFeedController extends PagedFeedController`；保持 account 切换/outbox。
- [ ] C7c `rg "ChangeNotifier" lib` 为 0。

## C7b 持久化增长

- [ ] C7b-1 `DownloadRecoveryStore` 记录上限（如 200，淘汰已完成项）或增量写入（code review 决定）；
