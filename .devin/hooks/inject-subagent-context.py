...
def _parse_hook_input(input_data: dict) -> tuple[str, str, dict]:
    ...
    # Devin: run_subagent tool carries (profile, task); profile names the
    # custom subagent definition under .devin/agents/.
    if tool_name == "run_subagent":
        return (
            _string_value(tool_input.get("profile")),
            _string_value(tool_input.get("task")),
            tool_input,
        )
...
def main():
    ...
    # Devin's run_subagent takes `task`, not `prompt`.
    prompt_key = "task" if "task" in tool_input and "prompt" not in tool_input else "prompt"
